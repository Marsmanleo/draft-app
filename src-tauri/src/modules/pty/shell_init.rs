...
fn apply_common(cmd: &mut CommandBuilder, cwd: Option<String>, blocks: bool) {
    ...
    cmd.env("DRAFT_TERMINAL", "1");
    if blocks {
        cmd.env("DRAFT_BLOCKS", "1");
    }
...
mod unix {
    ...
    pub fn build(cwd: Option<String>, blocks: bool) -> Result<CommandBuilder, String> {
        ...
        match shell {
            ...
            Shell::Fish => {
                ...
                if blocks {
                    cmd.arg("-C");
                    cmd.arg("functions -q __draft_install_prompt; and __draft_install_prompt");
                }
            }
            Shell::Other => {
                log::info!(
    ...
    fn integration_root() -> Result<PathBuf, String> {
        ...
        let root = home.join(".cache").join("draft").join("shell-integration");
    ...
    fn prepare_fish_conf_d() -> Result<(), String> {
        ...
        fs::create_dir_all(&dir).map_err(|e| format!("create {}: {e}", dir.display()))?;
        write_if_changed(&dir.join("draft.fish"), FISH_INIT)?;
        Ok(())
    }
...
mod windows {
        ...
        match (shell_kind, integration) {
            (
                ...
                if let Some(user_zdotdir) = user_zdotdir {
                    args.push(format!("DRAFT_USER_ZDOTDIR={user_zdotdir}"));
                }
    ...
    fn prepare_wsl_integration_dir(distro: &str, shell: &str) -> Result<(String, PathBuf), String> {
        ...
        let linux_dir = format!(
            "{}/.cache/draft/shell-integration/{shell}",
    ...
    fn prepare_wsl_fish_conf_d(distro: &str) -> Result<(), String> {
        ...
        let unc_dir = crate::modules::workspace::wsl_path_to_unc(distro, &linux_dir);
        fs::create_dir_all(&unc_dir).map_err(|e| format!("create {}: {e}", unc_dir.display()))?;
        let unc_file = unc_dir.join("draft.fish");
    ...
    mod tests {
        ...
        fn builds_wsl_zsh_launch_spec_with_env_and_login() {
            let spec = build_wsl_launch_spec(
                ...
                ShellKind::Zsh,
                WslShellIntegration::Zsh {
                    zdotdir: "/home/vinicios/.cache/draft/shell-integration/zsh".into(),
                    user_zdotdir: None,
                },
            );
            assert_eq!(
                ...
                vec![
                    ...
                    "--exec".to_string(),
                    "env".to_string(),
                    "ZDOTDIR=/home/vinicios/.cache/draft/shell-integration/zsh".to_string(),
        ...
        fn builds_wsl_zsh_launch_spec_with_user_zdotdir_probe() {
            ...
            assert_eq!(
                ...
                vec![
                    ...
                    "--exec".to_string(),
                    "env".to_string(),
                    "DRAFT_USER_ZDOTDIR=/home/vinicios/.config/zsh".to_string(),
                    "ZDOTDIR=/home/vinicios/.cache/draft/shell-integration/zsh".to_string(),
                    "/usr/bin/zsh".to_string(),
                    "-l".to_string(),
                ]
        ...
        fn builds_wsl_bash_launch_spec_with_rcfile() {
            let spec = build_wsl_launch_spec(
                ...
                WslShellIntegration::Bash {
                    rcfile: "/home/vinicios/.cache/draft/shell-integration/bash/bashrc".into(),
                },
            );
            assert_eq!(
                spec.args,
                vec![
                    ...
                    "/bin/bash".to_string(),
                    "--rcfile".to_string(),
                    "/home/vinicios/.cache/draft/shell-integration/bash/bashrc".to_string(),
                    "-i".to_string(),
                ]
            );
        }
