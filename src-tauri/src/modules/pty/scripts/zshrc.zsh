...
# zsh, so we shadow $? into `_draft_ret`.

{
  _draft_user_zdotdir="${DRAFT_USER_ZDOTDIR:-$HOME}"
  [ -f "$_draft_user_zdotdir/.zshrc" ] && source "$_draft_user_zdotdir/.zshrc"
...
if [[ -z "$__DRAFT_HOOKS_LOADED" ]]; then
  __DRAFT_HOOKS_LOADED=1
  ...
  _draft_precmd() {
    local _draft_ret=$?
    printf '\e]133;D;%s\e\\' "$_draft_ret"
    printf '\e]7;file://%s%s\e\\' "${HOST}" "$(_draft_urlencode "$PWD")"
  ...
  _draft_preexec() {
    ...
    [[ -n "$DRAFT_BLOCKS" ]] && _draft_block_seen=1
    local cmd="${1//[[:cntrl:]]/ }"
    printf '\e]133;C;%s\e\\' "${cmd[1,256]}"
  }

  if (( $+functions[add-zsh-hook] )); then
    add-zsh-hook precmd _draft_precmd
    add-zsh-hook preexec _draft_preexec
  fi
