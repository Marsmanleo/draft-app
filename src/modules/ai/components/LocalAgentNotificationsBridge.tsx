...
export function LocalAgentNotificationsBridge() {
  ...
  useEffect(() => {
    ...
    if (status === "awaiting-approval") {
      fire("attention", "Draft needs your approval", "Approve a tool to continue");
    } else if (status === "error") {
      fire("error", "Draft run failed", error ?? undefined);
    } else if (status === "idle" && isBusy(was)) {
      fire("finished", "Draft finished", "Your task is ready");
    }
  }, [status, error]);
