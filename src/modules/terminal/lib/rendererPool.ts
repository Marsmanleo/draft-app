...
function getRecycler(): HTMLDivElement {
  ...
  el.setAttribute("data-draft-recycler", "");
  el.style.cssText =
...
function createSlot(): Slot {
  ...
  host.setAttribute("data-draft-slot", String(slots.length));
  ...
  if (!fast) {
    ...
    if (p.snapshot) {
      try {
        ...
        console.warn("[draft] snapshot replay failed:", e);
      }
    }
    if (p.altScreen) {
...
function serializeSlot(slot: Slot): SerializeOutput {
  ...
  try {
    ...
    console.warn("[draft] serialize failed:", e);
  }
  return {
    snapshot,
    cols: slot.term.cols,
...
function disposeSlot(slot: Slot): void {
  ...
  try {
    ...
    console.warn("[draft] slot dispose failed:", e);
  }
  slot.host.remove();
...
function attachWebgl(slot: Slot): void {
  ...
  try {
    ...
    console.warn("[draft-webgl] unavailable:", e);
  }
}
...
function disposeSlotWebgl(slot: Slot): void {
  ...
  try {
    ...
    console.warn("[draft-webgl] dispose failed:", e);
  }
  try {
    const r = (
