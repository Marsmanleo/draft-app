...
function distroCommand(key: DistroKey, version: string): string {
  switch (key) {
    case "arch":
      return "yay -S draft-bin";
    case "debian":
      return `sudo apt install ./Draft_${version}_amd64.deb`;
    case "fedora":
      return `sudo dnf install ./Draft-${version}-1.x86_64.rpm`;
  }
}
          ...
          <DialogTitle>
            {ready
              ...
              : downloading
                ? "Downloading update…"
                : manual
                  ? `Draft v${manual.version} is available`
                  : `Draft v${update?.version} is available`}
          </DialogTitle>
          <DialogDescription>
            {ready
              ? "Restart Draft to finish installing."
              : downloading
                ? progress !== null
